...
describe('SearchIndex.search()', () => {
    describe('when a matching model exists', () => {
        ...
        test('returns the expected model', () => {
            expect(result).toStrictEqual([
                new SearchResult(SimpleModelWithSearchIndex.fromData(model1.toSearchData()), 0.08699614330652679),
            ]);
        });
    });
    ...
    describe('when multiple matching models exist', () => {
        ...
        test('returns the expected model', () => {
            expect(result).toStrictEqual([
                new SearchResult(SimpleModelWithSearchIndex.fromData(model1.toSearchData()), 0.08699614330652679),
                new SearchResult(SimpleModelWithSearchIndex.fromData(model2.toSearchData()), 0.08699614330652679),
            ]);
        });
    });
});
